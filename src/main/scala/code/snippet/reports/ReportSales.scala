	...
	def simpleReportCommandConference(xhtml:NodeSeq):NodeSeq = {
		paymentsFiltered.flatMap(p => 
			bind("f", xhtml,"cashier" -> Text(p.cashier.obj.get.idForCompany.is.toString),
							"date" -> Text(p.datePayment.is.getTime.toString),	
							"customerid" -> Text(p.customer.is.toString),
							"customername" -> <a style='line-height: 105%' href={"/customer/edit?id="+p.customer.is.toString+""} target='_customer_maste'>{""+p.customerShortName+""}</a>,
							"command" -> Text(p.command.is.toString),
							"user" -> <p style='line-height: 105%'>{""+treatmentsProfAsText(p.treatments.toList)+""}</p>,
							"treatments" -> <p style='line-height: 105%'>{""+treatmentsAsText(p.treatments.toList)+""}</p>,
							"payments" -> <p style='line-height: 105%'>{""+p.details.map(_.typePaymentTranslated+", ").foldLeft("")(_ + _)+""}</p>,
							"total" ->Text(p.value.is.toString),
							...
							"receiptprof" -> <a target="_command_maste" href={"/financial_cashier/expense_receipt?command="+p.command.is.toString+"&date="+Project.dateToStr(p.datePayment.is)+"&customerId="+p.customer.is.toString+"&prof=true"}><img alt="Imprimir recibo do profissional" src="/images/print.png" width="24"/></a>,
							"receipt" -> <a target="_command_maste" href={"/financial_cashier/expense_receipt?command="+p.command.is.toString+"&date="+Project.dateToStr(p.datePayment.is)+"&customerId="+p.customer.is.toString+"&prof=false"}><img alt="Imprimir recibo do estabelecimento" src="/images/print.png" width="24"/></a>,
